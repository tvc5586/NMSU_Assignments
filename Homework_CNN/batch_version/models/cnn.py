...
class CNN(object):
    ...
    def __init__(
        ...
        for i in range(self.nlayers):
            # First layer
            if i == 0:
                self.convolutional_layers.append(Conv1d(num_input_channels,
                                                        num_channels[i],
                                                        kernel_sizes[i],
                                                        strides[i],
                                                        0,
            ...
            # Subsequent layers
                ...
                self.convolutional_layers.append(Conv1d(num_channels[i - 1],
                                                        num_channels[i],
                                                        kernel_sizes[i],
                                                        strides[i],
                                                        0,
        ...
        for i in range(self.nlayers):
            # First layer
            if i == 0:
                output_width = np.floor((input_width - kernel_sizes[i]) / strides[i]) + 1
            
            # Subsequent layers
            else:
                output_width = np.floor((output_width - kernel_sizes[i]) / strides[i]) + 1


        # create the linear layer
        self.flatten = Flatten()
        self.linear_layer = Linear(int(output_width * num_channels[-1]), num_linear_neurons)
        
        # Don't change this - START
        self.linear_layer.W = linear_weight_init_fn(
            num_linear_neurons, int(num_channels[-1] * output_width)
        )
    ...
    def forward(self, A):
        ...
        # After last convolutional layer
    ...
    def backward(self, labels):
        ...
        for i, conv_layer in reversed(list(enumerate(self.convolutional_layers))):
